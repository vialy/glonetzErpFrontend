    ...
    <div className="min-h-0 flex-1 px-4 pb-28 pt-4 md:px-6 md:pb-10 lg:px-8">
      ...
      <AdminPageHeader
        ...
        subtitle={t("adm_class_fiche_session_sub")
          .replace("{session}", cls.session)
          .replace("{ref}", String(cls.learnersCount))
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-border/80 bg-card p-4 shadow-sm">
          ...
          <p className="mt-1 text-xl font-bold tabular-nums">{formatMoney(cls.tuitionAmount)}</p>
        </div>
        <div className="rounded-2xl border border-border/80 bg-card p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("adm_class_fiche_encashed")}</p>
          <p className="mt-1 text-xl font-bold tabular-nums text-emerald-700">{formatMoney(cls.totalPaid)}</p>
        </div>
        <div className="rounded-2xl border border-border/80 bg-card p-4 shadow-sm">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-2xl border bg-muted/30 p-4 text-xs text-muted-foreground">
            ...
            <p className="mt-2">
              {t("adm_class_fiche_period_from")} <span className="font-mono text-foreground">{cls.periodStart}</span>{" "}
