...
export function MockManagerPaymentsPage({ embedded = false }: { embedded?: boolean }) {
  const { t } = useLocale()
    ...
    <div className="space-y-4">
      <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
        {!embedded ? (
          ...
          <div className="flex justify-end border-b border-border/60 px-4 py-3 sm:px-5">
            <button
              type="button"
              onClick={exportCsv}
              className="inline-flex items-center gap-2 rounded-xl border border-border/60 bg-background px-3 py-2 text-xs font-semibold shadow-sm transition hover:bg-muted/40"
            >
              <Download className="size-3.5" />
              {t("fin_mgr_pay_export")}
            </button>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-border/60 bg-muted/10 p-3 sm:grid-cols-4 sm:p-4">
                      ...
                      <div className="flex flex-wrap justify-end gap-2">
                        <button
                          ...
                          onClick={() => {
                            ...
                            const result = settleManagerTransfer(op.id, {
                              status: "success",
                              externalTransactionId: extRef,
                            })
                            if (!result.ok) {
                              toast({
                                title: t("fin_mgr_pay_toast_impossible"),
                        ...
                        <button
                          ...
                          onClick={() => {
                            const result = settleManagerTransfer(op.id, {
                              status: "failed",
                              failureReason: t("fin_mgr_pay_fail_reason"),
                            })
                            if (!result.ok) {
                              toast({
                                title: t("fin_mgr_pay_toast_impossible"),
                            ...
                            toast({
                              title: t("fin_mgr_pay_toast_failed_title"),
                              description: t("fin_mgr_pay_toast_failed_desc"),
                            })
                          }}
      ...
      <div className="space-y-3 lg:hidden">
        {pagedOps.length === 0 ? (
          ...
          pagedOps.map((op) => (
            <div key={op.id} className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      ...
      <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
          {(managerWalletSnapshots.length > 0
            ...
            <div key={snap.managerId} className="rounded-xl border border-border/80 bg-muted/15 px-4 py-3">
