...
function statusBadge(p: AdminPaymentItem) {
  if (p.status === "success") {
    return (
...
function adminPaymentMethodLabel(method: AdminPaymentItem["method"]) {
  if (method === "Especes") return "Especes (guichet)"
  return method === "MTN" ? "MTN Mobile Money" : "Orange Money"
}

function adminPaymentStatusLabel(status: AdminPaymentItem["status"]) {
  if (status === "success") return "Valide"
  if (status === "pending") return "En attente"
  return "Saisie manuelle"
}

function PaymentReceiptPremiumCard({ p, previewLine }: { p: AdminPaymentItem; previewLine: string }) {
  const ref = p.operatorReference ?? p.id
  const rows: { label: string; value: string }[] = [
    { label: "Apprenant", value: p.learnerName },
    { label: "Classe", value: p.className },
    { label: "Date", value: p.createdAt },
    { label: "Mode de paiement", value: adminPaymentMethodLabel(p.method) },
    { label: "Statut", value: adminPaymentStatusLabel(p.status) },
    { label: "ID transaction", value: p.id },
  ]
  if (p.operatorReference) rows.push({ label: "Reference operateur", value: p.operatorReference })
  if (p.learnerId) rows.push({ label: "ID apprenant", value: p.learnerId })
      ...
      <div className="relative px-6 pb-8 pt-9 sm:px-10 sm:pb-10 sm:pt-11">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            ...
            <h3 className="font-serif text-2xl font-bold tracking-tight text-foreground sm:text-[1.65rem]">Recu de paiement</h3>
          ...
          <div className="shrink-0 rounded-2xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/15 to-teal-500/5 px-5 py-4 text-right shadow-inner">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Montant verse</p>
...
export function PaymentsListPage({
  ...
  const validatePending = async (id: string) => {
    ...
    try {
      ...
      setBanner({
        type: "success",
        text: updated ? "Paiement confirme et marque comme valide." : "Ce paiement n'etait pas en attente.",
      })
    } catch {
      setBanner({ type: "error", text: "Action impossible pour le moment." })
    } finally {
      setActingId(null)
    }
  }
  ...
  function exportCsv() {
    if (filtered.length === 0) {
      setBanner({ type: "error", text: "Aucune ligne a exporter avec les filtres actuels." })
      return
    }
    ...
    const a = document.createElement("a")
    a.href = url
    a.download = `paiements-admin-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
    URL.revokeObjectURL(url)
    setBanner({ type: "success", text: `${filtered.length} ligne(s) exportee(s).` })
  }
      ...
      <div className="flex-1 px-4 pt-4 pb-28 md:px-6 md:pb-10 lg:px-8">
        ...
        <div className="mt-5 overflow-hidden rounded-2xl border border-border/80 bg-card shadow-md">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {hasActiveFilters ? (
                ...
                <span className="rounded-full bg-emerald-500/20 px-2.5 py-1 text-[11px] font-medium text-emerald-100">
                  Vue complete ({payments.length} paiements)
                </span>
              )}
              <button
                type="button"
          ...
          <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-2">
            <div className="space-y-3 rounded-xl border border-border/60 bg-muted/20 p-3 sm:p-4">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Reference, apprenant, classe, ID..."
                  value={query}
            ...
            <div className="space-y-3 rounded-xl border border-border/60 bg-muted/20 p-3 sm:p-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as typeof statusFilter)}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="success">Valide</SelectItem>
              ...
              <div className="space-y-1.5">
                <Label className="text-[11px] text-muted-foreground">Classe (libelle)</Label>
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-border/60 bg-muted/30 px-4 py-2.5 text-xs text-muted-foreground sm:px-5">
            <span className="font-medium text-foreground">
              {kpis.count} paiement{kpis.count !== 1 ? "s" : ""} dans le perimetre
            </span>
            {kpis.count === 0 ? (
        ...
        <div className="mt-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <AdminKpiCard
              label="Montant total"
              value={formatFcfa(kpis.total)}
              hint="Somme des montants filtres"
              icon={<CreditCard className="size-4 text-teal-600" />}
            />
            <AdminKpiCard label="Transactions" value={String(kpis.count)} hint="Lignes dans le perimetre" />
            <AdminKpiCard label="Validees" value={String(kpis.success)} hint="Statut valide" />
            <AdminKpiCard
              label="En attente"
              value={String(kpis.pending)}
              hint="A traiter ou valider"
              ...
              {paged.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="p-0">
                    <AdminEmptyState
                      ...
                      description="Ajustez les filtres ou verifiez les donnees mock."
                      action={
                ...
                paged.map((p) => (
                  <TableRow key={p.id} className="group">
                    ...
                    <TableCell className="text-right tabular-nums">{formatFcfa(p.amount)}</TableCell>
                        ...
                        {canDownloadAdminPaymentReceipt(p) ? (
                          <>
                            ...
                            <Button
                              ...
                              disabled={receiptBusyId !== null}
                              aria-label="Telecharger le recu PDF"
          ...
          {paged.length === 0 ? (
            ...
            paged.map((p) => (
              ...
              >
                ...
                <p className="mt-3 text-lg font-bold tabular-nums">{formatFcfa(p.amount)}</p>
                ...
                <div className="mt-3 flex flex-col gap-2">
                  {p.status === "pending" ? (
                    <Button
                      ...
                      onClick={() => void validatePending(p.id)}
                    >
                      {actingId === p.id ? "Validation..." : "Valider le paiement"}
                    </Button>
                  ) : null}
                  {canDownloadAdminPaymentReceipt(p) ? (
            ...
            <div className="relative mx-auto w-full max-w-xl md:max-w-2xl">
              ...
              <div className="relative overflow-hidden rounded-[24px] border border-white/20 bg-gradient-to-b from-slate-950/95 via-slate-900/98 to-slate-950 shadow-[0_40px_100px_-40px_rgba(6,78,59,0.55)] dark:shadow-black/60">
                ...
                <div className="max-h-[min(58vh,560px)] overflow-y-auto overscroll-contain px-4 pb-2 pt-4 sm:px-6 sm:pb-3 sm:pt-5">
                  {receiptViewPayment ? (
                    <PaymentReceiptPremiumCard p={receiptViewPayment} previewLine={receiptPreviewLine} />
                  ) : null}
                </div>
                ...
                <DialogFooter className="relative gap-2 border-t border-white/10 bg-black/25 px-4 py-4 backdrop-blur-md sm:justify-between sm:px-6 sm:py-5">
                  <p className="hidden text-[11px] text-slate-400 sm:mr-auto sm:block">
                    Le PDF telecharge reprend les memes informations.
                  </p>
