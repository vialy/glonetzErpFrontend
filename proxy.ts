...
function staffHomePath(role: string) {
  if (role === "collaborateur") return "/dashboard/collaborateur/apprenants"
  return "/dashboard"
}
...
export function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith("/dashboard")) {
    ...
    if (pathname.startsWith("/dashboard/admin") && session.role !== "admin") {
      return NextResponse.redirect(new URL(staffHomePath(session.role), request.url))
    }
    ...
    if (pathname.startsWith("/dashboard/collaborateur") && session.role !== "collaborateur") {
      return NextResponse.redirect(new URL(staffHomePath(session.role), request.url))
    }
